...
namespace BlazeAISpace
{
    public class AttackStateBehaviour : MonoBehaviour
    {
        ...
        [Min(0),
         Tooltip(
             "The idle distance between the AI and the target waiting to attack. If distance between the AI and the target is more than this value the AI will chase the player.")]
        ...
        [Min(0),
         Tooltip(
             "Keep rotating/facing enemy while attacking. Good for ranged enemies when you always want them to be looking at the target.")]
        ...
        void Update()
        {
            ...
            if (isSearching)
            {
                if (blaze.MoveTo(searchLocation, alertStateBehaviour.moveSpeed, alertStateBehaviour.turnSpeed,
            ...
            if (turnToTarget)
            {
                ...
                if (turningToTarget)
                {
                    ...
                    if (!blaze.TurnTo(enemyPos, blaze.waypoints.leftTurnAnimAlert, blaze.waypoints.rightTurnAnimAlert,
                            blaze.waypoints.turningAnimT, 7, useTurnAnims))
                    {
                        return;
                    }
        ...
        void Attack()
        {
            ...
            if (Physics.Raycast(transform.position + blaze.centerPosition, dir, out hit, Mathf.Infinity, layers))
            {
                ...
                if (!hit.transform.IsChildOf(blaze.enemyToAttack.transform) &&
                    !blaze.enemyToAttack.transform.IsChildOf(hit.transform))
                {
                    IdlePosition();
                    return;
                }
        ...
        void CallOthers()
        {
            ...
            int callOthersNum =
                Physics.OverlapSphereNonAlloc(transform.position, callRadius, callOthersColl, agentLayersToCall);
        ...
        void StrafeMovement(int direction)
        {
            ...
            if (strafeCheckPathElapsed >= 5)
            {
                ...
                Vector3 goToPoint = blaze.ValidateYPoint((transform.position + blaze.centerPosition) +
                                                         (transformDir * (blaze.navmeshAgent.radius * 2 +
                                                                          blaze.navmeshAgent.height)));
            ...
            if (Physics.SphereCast(transform.position + blaze.centerPosition, 0.3f, transformDir, out hit,
                    (blaze.navmeshAgent.radius * 2) + blaze.navmeshAgent.height / 2, layersToHit))
            {
        ...
        void MoveBackwards(Vector3 target)
        {
            ...
            if (Physics.Raycast(transform.position + blaze.centerPosition, -transform.forward, out hit,
            ...
            if (isStrafing)
            {
                ...
                {
                    Vector3 checkPoint = new Vector3(backupPoint.x, transform.position.y,
